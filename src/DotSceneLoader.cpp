...
////////////////////////////////////////////////////
//modificando para anadir soporte a fisicas
/////////////////////////////////////////////////////////////////////
void DotSceneLoader::processPhysics(rapidxml::xml_node<>* XMLNode, 
	...
	Ogre::String colliderType = getAttrib(XMLNode, "collider_type");
	int collType = 0;
	if(colliderType == "mesh"){
		collType = COLLISION_MESH;
		
	}
	if(colliderType == "primitiveBox"){
		collType = PRIMITIVE_BOX;
	}
	if(colliderType == "primitiveSphere"){
		collType = PRIMITIVE_BOX;
	}
		
	Ogre::String collisionMesh = getAttrib(XMLNode, "collision_mesh");

	bool isGhost = false;
	isGhost = getAttribBool(XMLNode, "isGhost");

	bool isConvex = false;
	isConvex = getAttribBool(XMLNode, "isConvex");

	m_pPhysicsMgr->createPhysicObject(pNode, physType, collType, 0, 
		collisionMesh, isGhost, 0, 0, 0, Ogre::Vector3::ZERO, 
		Ogre::Quaternion::IDENTITY, isConvex);
}
////////////////////////////////////////////////////

////////////////////////////////////////////////////
//modificando para anadir soporte a animaciones
/////////////////////////////////////////////////////////////////////
void DotSceneLoader::processAnimation(rapidxml::xml_node<>* XMLNode, 
									Ogre::SceneNode *pNode){
	
	Ogre::String name = getAttrib(XMLNode, "animFile");
	bool invert = getAttribBool(XMLNode, "invert", false);
	float speed = getAttribReal(XMLNode, "speed", 1);
	int startFrame = getAttribReal(XMLNode, "startFrame", 0);


	AnimationLoader animLoader;
	animLoader.parseAnimation(name, m_sGroupName, pNode, mSceneMgr, 
		invert, speed, startFrame);
}
//////////////////////////////////////////////////

//procesar mis game objects
void DotSceneLoader::processUserDataReference(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
	...
	//procesar si es un checkpoint
	pElement = XMLNode->first_node("checkpoint");
	if(pElement){
		CheckPoint* checkpoint = new CheckPoint(pParent);
		Ogre::String type = getAttrib(pElement, "type", "");
		if(type == "default"){
			checkpoint->setDefaultCheckPoint();
		}
	}

    // Procesar si es una trampa
    pElement = XMLNode->first_node("trap");
    if(pElement){
		Ogre::String trapType = getAttrib(pElement, "type", "");
        TrapFactory* trapFactory = new TrapFactory();
		if(trapType == "masher"){
			trapFactory->createMasher(pParent);
		}
		if(trapType == "slasher"){
			trapFactory->createSlasher(pParent);
		}
		if(trapType == "deathMill"){
			trapFactory->createDeathMill(pParent);
		}
		if(trapType == "spikes"){
			trapFactory->createSpikes(pParent);	
		}

    }

	// Procesar si es una plataform
    pElement = XMLNode->first_node("platform");
	if(pElement){
		MovingPlatform* platform = new MovingPlatform(pParent);
	}

	//procesar objetos que se pueden coger
	 pElement = XMLNode->first_node("collectible");
    if(pElement){
		Ogre::String collectibleType = getAttrib(pElement, "type", "");
		if(collectibleType == "skystone"){
			Skystone* stone = new Skystone(pParent);
		}
	}

	//procesar el limite del nivel (deathTrigger)
	pElement = XMLNode->first_node("deathTrigger");
	if(pElement){
		DeathTrigger* deathTrigger = new DeathTrigger(pParent);
	}

	/////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
	//procesar el boss final
	pElement = XMLNode->first_node("Boss");
	if(pElement){
		BossManager::getSingletonPtr()->createBoss(pParent);
	
	}

	
	//procesar interruptor
	pElement = XMLNode->first_node("switch");
	if(pElement){
		Switch* finalSwitch = new Switch(pParent, mSceneMgr);
	}


	//procesar portal
	pElement = XMLNode->first_node("portal");
	if(pElement){
		Portal* portal = new Portal(pParent, mSceneMgr);
	}
...
DotSceneLoader::DotSceneLoader() : mSceneMgr(0), mTerrainGroup(0), m_pPhysicsMgr(0)
{
...
void DotSceneLoader::parseDotScene(const Ogre::String &SceneName, const Ogre::String &groupName, 
								   Ogre::SceneManager *yourSceneMgr, PhysicsManager* physicsMgr,
...
void DotSceneLoader::processScene(rapidxml::xml_node<>* XMLRoot)
{
    ...
    // Process light (?)
    pElement = XMLRoot->first_node("lights");
    if(pElement)
	...
	*/
	...
	if(sValue == "directional")	{
		//pLight->setDirection(parseVector3(pElement));
			//pLight->setDiffuseColour(Ogre::ColourValue(1, 1, 1));
			//pLight->setSpecularColour(Ogre::ColourValue(0, 0, 0.3));
...
void DotSceneLoader::processNode(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    while(pElement)
    {
		...
		// Procesar physics, implementacion propia
    pElement = XMLNode->first_node("physics");
    if(pElement)
    {
        processPhysics(pElement, pNode);
	...
	// Procesar animaciones, implementacion propia
...
void DotSceneLoader::processEntity(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
	...
	//para crear la geometria estatica
	if(isStatic){
		Ogre::StaticGeometry *sg = mSceneMgr->createStaticGeometry("STATIC"+name);
		sg->addSceneNode(pParent);
		mSceneMgr->destroySceneNode(pParent);
		sg->setCastShadows(castShadows);
		sg->build();
	}
}
...
void DotSceneLoader::processSkyBox(rapidxml::xml_node<>* XMLNode)
{
    ...
    // Setup the sky box
    mSceneMgr->setSkyBox(active, material, distance, drawFirst, rotation, m_sGroupName);
}
