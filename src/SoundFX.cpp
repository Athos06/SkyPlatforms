...
Ogre::Resource (creator, resource_name, handle, resource_group, isManual,
		loader)
{
...
int
SoundFX::play (int loop, int repChannel)
{

	if(repChannel)
		m_channel = repChannel;
  int
    channel;
  Ogre::LogManager * pLogManager = Ogre::LogManager::getSingletonPtr ();

  if ((channel = Mix_PlayChannel (m_channel, _pSound, loop)) == -1)
    {
      pLogManager->
	logMessage
	("SoundFX::play() Imposible reproducir el efecto de sonido.");
      throw (Ogre::
...
SoundFX::loadImpl ()
{
  ...
  info =
    Ogre::ResourceGroupManager::getSingleton ().findResourceFileInfo (mGroup,
								      mName);
  for (Ogre::FileInfoList::iterator i = info->begin (); i != info->end ();
       ++i)
    {
  ...
  if (_path == "")
    {
      pLogManager->
	logMessage ("SoundFX::loadImpl() Imposible encontrar el recurso.");
      throw (Ogre::
  ...
  if ((_pSound = Mix_LoadWAV (_path.c_str ())) == NULL)
    {
      pLogManager->
	logMessage ("SoundFX::loadImpl() Imposible cargar el efecto.");
      throw (Ogre::
	     Exception (Ogre::Exception::ERR_INTERNAL_ERROR,
			"Imposible cargar el efecto", "SoundFX::loadImpl()"));
    }

  // Cálculo del tamano del recurso de sonido.
  std::ifstream stream;
  char byteBuffer;
